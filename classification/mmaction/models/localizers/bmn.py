...
class BMN(BaseLocalizer):
    ...
    def _boundary_matching_layer(self, x):
        ...
        out = torch.matmul(x, self.sample_mask).reshape(
            input_size[0], input_size[1], self.num_samples, self.tscale, self.tscale
        )
        return out
    ...
    def forward_test(self, raw_feature, video_meta):
        ...
        for idx in range(1, self.tscale - 1):
            if (
                ...
                and start_scores[idx] > start_scores[idx - 1]
            ):
                start_bins[idx] = 1
            ...
            if (
                ...
                and end_scores[idx] > end_scores[idx - 1]
            ):
                end_bins[idx] = 1
        ...
        for idx in range(self.tscale):
            for jdx in range(self.tscale):
                ...
                if (
                    end_index < self.tscale
                    and start_bins[start_index] == 1
                    and end_bins[end_index] == 1
                ):
        ...
        output = [
            dict(video_name=video_info["video_name"], proposal_list=proposal_list)
        ]
        return output
    ...
    def generate_labels(self, gt_bbox):
        ...
        for every_gt_bbox in gt_bbox:
            ...
            for start, end in every_gt_bbox:
                ...
                current_gt_iou_map = temporal_iou(
                    self.match_map[:, 0], self.match_map[:, 1], start, end
                )
                current_gt_iou_map = np.reshape(
                    current_gt_iou_map, [self.tscale, self.tscale]
                )
    ...
    def forward(self, raw_feature, gt_bbox=None, video_meta=None, return_loss=True):
        ...
        if return_loss:
            ...
            return self.forward_train(
                raw_feature, label_confidence, label_start, label_end
            )
    ...
    def _get_interp1d_bin_mask(
        seg_tmin, seg_tmax, tscale, num_samples, num_samples_per_bin
    ):
        """Generate sample mask for a boundary-matching pair."""
        plen = float(seg_tmax - seg_tmin)
        plen_sample = plen / (num_samples * num_samples_per_bin - 1.0)
        total_samples = [
            seg_tmin + plen_sample * i for i in range(num_samples * num_samples_per_bin)
        ]
        p_mask = []
        for idx in range(num_samples):
            bin_samples = total_samples[
                idx * num_samples_per_bin : (idx + 1) * num_samples_per_bin
            ]
