...
class TEM(BaseLocalizer):
    ...
    def generate_labels(self, gt_bbox):
        ...
        for every_gt_bbox in gt_bbox:
            ...
            gt_len_pad = np.maximum(
                1.0 / self.temporal_dim, self.boundary_ratio * gt_lens
            )
...
class PEM(BaseLocalizer):
    ...
    def forward_train(self, bsp_feature, reference_temporal_iou):
        ...
        temporal_iou_loss = F.smooth_l1_loss(
            anchors_temporal_iou, reference_temporal_iou
        )
        temporal_iou_loss = torch.sum(
            temporal_iou_loss * temporal_iou_weights
        ) / torch.sum(temporal_iou_weights)
    ...
    def forward(
        ...
        return self.forward_test(
            bsp_feature, tmin, tmax, tmin_score, tmax_score, video_meta
        )
