...
class NL3DWrapper(nn.Module):
    ...
    def __init__(self, block, num_segments, non_local_cfg=dict()):
        ...
        self.non_local_block = NonLocal3d(
            self.block.conv3.norm.num_features, **self.non_local_cfg
        )
    ...
    def forward(self, x):
        ...
        n, c, h, w = x.size()
        x = (
            x.view(n // self.num_segments, self.num_segments, c, h, w)
            .transpose(1, 2)
            .contiguous()
        )
    ...
    def make_temporal_pool(self):
        ...
        class TemporalPool(nn.Module):
            ...
            def forward(self, x):
                ...
                x = x.view(
                    n // self.num_segments, self.num_segments, c, h, w
                ).transpose(1, 2)
    ...
    def make_non_local(self):
        ...
        for i in range(self.num_stages):
            ...
            for idx, non_local in enumerate(non_local_stage):
                if non_local:
                    res_layer[idx] = NL3DWrapper(
                        res_layer[idx], self.num_segments, self.non_local_cfg
                    )
