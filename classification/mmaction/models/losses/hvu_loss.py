...
class HVULoss(BaseWeightedLoss):
    ...
    def __init__(
        self,
        categories=("action", "attribute", "concept", "event", "object", "scene"),
        ...
        for i in range(len(self.category_nums) - 1):
            self.category_startidx.append(
                self.category_startidx[-1] + self.category_nums[i]
            )
    ...
    def _forward(self, cls_score, label, mask, category_mask):
        ...
        if self.loss_type == "individual":
            losses = {}
            loss_weights = {}
            for name, num, start_idx in zip(
                self.categories, self.category_nums, self.category_startidx
            ):
                ...
                if self.with_mask:
                    ...
                    if torch.sum(category_mask_i) < 0.5:
                        losses[f"{name}_LOSS"] = torch.tensor(0.0).cuda()
            ...
            raise ValueError(
                "loss_type should be 'all' or 'individual', "
