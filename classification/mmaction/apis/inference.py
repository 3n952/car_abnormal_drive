...
def init_recognizer(config, checkpoint=None, device="cuda:0", use_frames=False):
    ...
    if (use_frames and config.dataset_type != "RawframeDataset") or (
        not use_frames and config.dataset_type != "VideoDataset"
    ):
        input_type = "rawframes" if use_frames else "video"
        raise RuntimeError(
            "input data type should be consist with the "
...
def inference_recognizer(
    model, video_path, label_path, use_frames=False, outputs=None, as_tensor=True
):
    ...
    if osp.isfile(video_path) and use_frames:
        raise RuntimeError(f"'{video_path}' is a video file, not a rawframe directory")
    if osp.isdir(video_path) and not use_frames:
        raise RuntimeError(f"'{video_path}' is a rawframe directory, not a video file")
    ...
    if use_frames:
        ...
        total_frames = len(
            list(
                filter(
                    lambda x: re.match(pattern, x) is not None, os.listdir(video_path)
                )
            )
        )
        ...
        data = dict(
            filename=video_path, label=-1, start_index=start_index, modality="RGB"
        )
