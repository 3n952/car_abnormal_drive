...
class TemporalInterlace(nn.Module):
    ...
    def __init__(self, in_channels, num_segments=3, shift_div=1):
        ...
        self.offset_net = OffsetNet(
            in_channels // shift_div, self.deform_groups, num_segments
        )
    ...
    def forward(self, x):
        ...
        x_descriptor = x[:, :num_folds, :, :].view(
            num_batches, self.num_segments, num_folds, h, w
        )
...
class ResNetTIN(ResNetTSM):
    ...
    def __init__(self, depth, num_segments=8, is_tin=True, shift_div=4, **kwargs):
    ...
    def make_temporal_interlace(self):
        ...
        self.layer1 = make_block_interlace(
            self.layer1, num_segment_list[0], self.shift_div
        )
        self.layer2 = make_block_interlace(
            self.layer2, num_segment_list[1], self.shift_div
        )
        self.layer3 = make_block_interlace(
            self.layer3, num_segment_list[2], self.shift_div
        )
        self.layer4 = make_block_interlace(
            self.layer4, num_segment_list[3], self.shift_div
        )
