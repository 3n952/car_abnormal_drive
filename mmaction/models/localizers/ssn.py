    ...
    def forward_test(
        ...
        **kwargs
        ...
        for idx in range(0, num_ticks, minibatch_size):
            chunk = imgs[:, idx : idx + minibatch_size, :, :, :].view(
                (-1,) + imgs.shape[2:]
            )
        ...
        (output, activity_scores, completeness_scores, bbox_preds) = self.cls_head(
        ...
        if bbox_preds is not None:
            ...
            bbox_preds[:, :, 0] = (
                bbox_preds[:, :, 0] * reg_norm_consts[1, 0] + reg_norm_consts[0, 0]
            )
            bbox_preds[:, :, 1] = (
                bbox_preds[:, :, 1] * reg_norm_consts[1, 1] + reg_norm_consts[0, 1]
            )
