...
class LFBInferHead(nn.Module):
    ...
    def __del__(self):
        assert len(self.all_features) == len(
            self.all_metadata
        ...
        _lfb_file_path = osp.normpath(
            osp.join(self.lfb_prefix_path, f"_lfb_{self.dataset_mode}_{rank}.pkl")
        )
        torch.save(_lfb, _lfb_file_path)
        print(
            f"{len(self.all_features)} features from {len(_lfb)} videos "
        ...
        for rank_id in range(world_size):
            _lfb_file_path = osp.normpath(
                osp.join(
                    self.lfb_prefix_path, f"_lfb_{self.dataset_mode}_{rank_id}.pkl"
                )
            )
