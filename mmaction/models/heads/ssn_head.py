...
class STPPTrain(nn.Module):
    ...
    def _extract_stage_feature(
        stage_feat, stage_parts, num_multipliers, scale_factors, num_samples
    ):
        ...
        for stage_part in stage_parts:
            ticks = torch.arange(0, stage_len + 1e-5, stage_len / stage_part).int()
            for i in range(stage_part):
                part_feat = (
                    stage_feat[:, ticks[i] : ticks[i + 1], :].mean(dim=1)
                    / num_multipliers
                )
...
class STPPTest(nn.Module):
    ...
    def __init__(self, num_classes, use_regression=True, stpp_stage=(1, (1, 2), 1)):
        ...
        if self.use_regression:
            self.feat_dim = self.activity_score_len + self.num_multipliers * (
                self.complete_score_len + self.reg_score_len
            )
        else:
            self.feat_dim = (
                self.activity_score_len + self.num_multipliers * self.complete_score_len
            )
        ...
        self.complete_slice = slice(
            self.activity_slice.stop,
            self.activity_slice.stop + self.complete_score_len * self.num_multipliers,
        )
        self.reg_slice = slice(
            self.complete_slice.stop,
            self.complete_slice.stop + self.reg_score_len * self.num_multipliers,
        )

    @staticmethod
    def _pyramids_pooling(
        out_scores, index, raw_scores, ticks, scale_factors, score_len, stpp_stage
    ):
        ...
        for stage_idx, stage_cfg in enumerate(stpp_stage):
            ...
            for num_parts in stage_cfg:
                part_ticks = torch.arange(
                    tick_left, tick_right + 1e-5, (tick_right - tick_left) / num_parts
                ).int()
    ...
    def forward(self, x, proposal_ticks, scale_factors):
        ...
        out_activity_scores = torch.zeros(
            (num_ticks, self.activity_score_len), dtype=x.dtype
        )
        raw_activity_scores = x[:, self.activity_slice]

        out_complete_scores = torch.zeros(
            (num_ticks, self.complete_score_len), dtype=x.dtype
        )
...
class SSNHead(nn.Module):
    ...
    def prepare_test_fc(self, stpp_feat_multiplier):
        ...
        complete_weight = (
            self.completeness_fc.weight.data.view(
                self.completeness_fc.out_features, stpp_feat_multiplier, in_features
            )
            .transpose(0, 1)
            .contiguous()
            .view(-1, in_features)
        ...
        complete_bias = (
            self.completeness_fc.bias.data.view(1, -1)
            .expand(stpp_feat_multiplier, self.completeness_fc.out_features)
            .contiguous()
            .view(-1)
            / stpp_feat_multiplier
        )
        ...
        if self.use_regression:
            reg_weight = (
                self.regressor_fc.weight.data.view(
                    self.regressor_fc.out_features, stpp_feat_multiplier, in_features
                )
                .transpose(0, 1)
                .contiguous()
                .view(-1, in_features)
            ...
            reg_bias = (
                self.regressor_fc.bias.data.view(1, -1)
                .expand(stpp_feat_multiplier, self.regressor_fc.out_features)
                .contiguous()
                .view(-1)
                / stpp_feat_multiplier
            )
    ...
    def forward(self, x, test_mode=False):
        ...
        (activity_scores, completeness_scores, bbox_preds) = self.consensus(
            test_scores, proposal_tick_list, scale_factor_list
        )
