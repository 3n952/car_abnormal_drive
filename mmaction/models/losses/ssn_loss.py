...
class SSNLoss(nn.Module):
    ...
    def completeness_loss(
        ...
        # yapf:disable
        positive_pred = (
            pred[:, :positive_per_video, :].contiguous().view(-1, pred_dim)
        )  # noqa:E501
        incomplete_pred = (
            pred[:, positive_per_video:, :].contiguous().view(-1, pred_dim)
        )  # noqa:E501
        ...
        positive_loss = OHEMHingeLoss.apply(
            positive_pred,
            gt[:, :positive_per_video].contiguous().view(-1),
            1,
            1.0,
            positive_per_video,
        )
        ...
        return (positive_loss + incomplete_loss) / float(
            num_positives + num_incompletes
        )
    ...
    def forward(
        ...
        activity_indexer = (
            ((proposal_type == 0) + (proposal_type == 2)).nonzero().squeeze(1)
        )
        completeness_indexer = (
            ((proposal_type == 0) + (proposal_type == 1)).nonzero().squeeze(1)
        )

        total_ratio = (
            self.sampler.positive_ratio
            + self.sampler.background_ratio
            + self.sampler.incomplete_ratio
        )
        positive_per_video = int(
            self.sampler.num_per_video * (self.sampler.positive_ratio / total_ratio)
        )
        background_per_video = int(
            self.sampler.num_per_video * (self.sampler.background_ratio / total_ratio)
        )
        incomplete_per_video = (
            self.sampler.num_per_video - positive_per_video - background_per_video
        )
