...
def binary_logistic_regression_loss(
    reg_score, label, threshold=0.5, ratio_range=(1.05, 21), eps=1e-5
):
...
class BinaryLogisticRegressionLoss(nn.Module):
    ...
    def forward(
        self, reg_score, label, threshold=0.5, ratio_range=(1.05, 21), eps=1e-5
    ):
        ...
        return binary_logistic_regression_loss(
            reg_score, label, threshold, ratio_range, eps
        )
