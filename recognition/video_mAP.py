...
class testData(Dataset):
    def __init__(self, root, shape=None, transform=None, clip_duration=10, sampling_rate=1):
...
def video_mAP_traffic():
    ...
    CLASSES = ("방향지시등 불이행", "실선구간 차선변경", "동시 차로변경", "차선물기", "2개 차로 연속 변경", "정체구간 차선변경", "안전거리 미확보 차선변경")
                ...
                for i in range(output.size(0)):
                    ...
                    for cls_idx in range(num_classes):
                        ...
                        for b in range(n_boxes):
                            ...
                            cls_boxes[b][1] = max(float(boxes[b][1]-boxes[b][3]/2.0) * 720.0, 0.0)
                            cls_boxes[b][2] = min(float(boxes[b][0]+boxes[b][2]/2.0) * 1280.0, 320.0)
                            cls_boxes[b][3] = min(float(boxes[b][1]+boxes[b][3]/2.0) * 720.0, 240.0)
                    ...
                    for g in range(num_gts):
                        ...
                        gt_boxes.append(float(truths[g][1]-truths[g][3]/2.0) * 1280.0)
                        gt_boxes.append(float(truths[g][2]-truths[g][4]/2.0) * 720.0)
                        gt_boxes.append(float(truths[g][1]+truths[g][3]/2.0) * 1280.0)
                        gt_boxes.append(float(truths[g][2]+truths[g][4]/2.0) * 720.0)
